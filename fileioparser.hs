...
showVal (Func {params = args, vararg = varargs, body = body,
...
apply (Func params varargs body closure) args =
  if length params /= length args && varargs == Nothing
    then throwError $ NumArgs (toInteger $ length params) args
      ...
      let over = case varargs of
            Nothing   -> []
            Just str  -> [(str, List $ drop (length params) args)]
      paramEnv <- liftIO . bindVars closure $ over ++ zip params args
      last <$> mapM (eval paramEnv) body
apply badForm _ = throwError . NotFunction "Not a function" $ show badForm

primitiveBindings :: IO Env
primitiveBindings = do
  env <- nullEnv
  bindVars env $ map (\(var, func) -> (var, PrimitiveFunc func)) primitives
    ++ map (\(var, func) -> (var, IOFunc func)) ioPrimitives

makeFunc varargs env params body = return $ Func
  (map showVal params) varargs body env
...
makePort _ bad = throwError $ BadSpecialForm "Not a String" (List bad)
...
readProc [Port port]  = do
  ...
  liftThrows $ readExpr str
readProc bad = throwError $ BadSpecialForm "Not a Port" (List bad)

writeProc :: [LispVal] -> IOThrowsError LispVal
writeProc [obj]             = writeProc [obj, Port stdout]
writeProc [obj, Port port]  = liftIO $ hPrint port obj
                              >> (return $ Bool True)
writeProc bad = throwError $ BadSpecialForm "Bad arguments" (List bad)

readContents :: [LispVal] -> IOThrowsError LispVal
readContents [String filename] = liftM String $ liftIO $ readFile filename
readContents bad = throwError $ BadSpecialForm "Not a String" (List bad)
...
readAll bad = throwError $ BadSpecialForm "Not a String" (List bad)
-- IOFile end
